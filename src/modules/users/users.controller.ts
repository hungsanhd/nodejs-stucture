...
export default class UsersController {
  ...
  public getAll = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // .json(res.status(200).json({ status: 200, message: "ok" }));
    } catch (error) {
      next(error);
    }
  };
  ...
  public passwordRetrieval = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
    try {
      const model: RegisterDto = req.body;
      const user = await this.userService.passwordRetrieval(model);
      res.status(201).json({ status: 201, message: "Reset password success" });
    } catch (error) {
      next(error);
    }
  };
